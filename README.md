...
All commands accept a task description as an argument (e.g. `/council review the auth flow before launch`). They read `docs/PROJECT_BRIEF.md` and `AGENTS.md` for project context automatically.

For Cursor, Codex, Gemini, and Copilot, use the persona docs in `agentic personas/` directly as prompts.

## Persona Council

For multi-agent or multi-perspective work, use `/council` (Claude Code) or `agentic personas/agent-council-protocol.md` (all other tools).
